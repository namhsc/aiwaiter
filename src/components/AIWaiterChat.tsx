		...
		if (!messagesAI.length) {
			...
			setMessages([
				...
				...messagesAI.map((itemMess: ChatMessageAI) => {
					...
					// Check if AI response contains menu IDs and extract suggested items
					if (inforCretor.type === 'ai') {
						const menuIds = extractMenuIds(data_reply.content);
						const suggestedItemsFromIds = getMenuItemsByIds(menuIds);

						console.log('AI Response:', data_reply.content);
						console.log('Extracted Menu IDs:', menuIds);
						console.log('Suggested Items from IDs:', suggestedItemsFromIds);

						if (suggestedItemsFromIds.length > 0) {
							setSuggestedItems(suggestedItemsFromIds);
						}
					}
			...
			<div className="w-full max-w-[480px] flex flex-col bg-gradient-to-br from-[#FFF9F0] via-[#FFF9F0] to-[#FFF4E0]">
				...
				<div className="bg-gradient-to-r from-[#C4941D] to-[#D4A52D] border-b border-[#B8860B]/30 px-4 py-3 shadow-lg flex items-center gap-3 shrink-0">
					...
					<Button
						...
						variant="outline"
						size="sm"
						className="rounded-full bg-white/20 text-white border-white/30 hover:bg-white/30 hover:border-white/50 transition-all shadow-md w-24"
					>
						{showMenuOverlay ? (
							<>
								<Sparkles className="w-4 h-4 mr-1" />
								<span className="text-xs font-medium">AI Waiter</span>
							</>
						) : (
							<>
						...
						<button
							onClick={() => {
								...
								simpleTests.forEach((test, index) => {
									console.log(`\n--- Test ${index + 1}: "${test}" ---`);
									const ids = extractMenuIds(test);
									console.log('Extracted IDs:', ids);
									const items = getMenuItemsByIds(ids);
									console.log('Found items:', items.length);
								});

								// Test with the full response
...
Would you like me to recommend drinks or desserts to pair with any of these? 😊`;
								...
								console.log('\n--- Full Test Response ---');
								const menuIds = extractMenuIds(testResponse);
								const suggestedItemsFromIds = getMenuItemsByIds(menuIds);

								console.log('Final suggested items:', suggestedItemsFromIds);
								setSuggestedItems(suggestedItemsFromIds);
							}}
							className="flex items-center gap-1.5 bg-red-500 text-white px-2.5 py-1.5 rounded-lg shadow-md hover:bg-red-600 active:scale-95 transition-all text-xs"
						>
							Test
						</button>
