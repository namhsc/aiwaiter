...
const hasOrderingIntent = (message: string): boolean => {
	...
	// EXCLUDE non-ordering intents first (checkout, cart queries, menu browsing)
...
export const generateAIResponse = (
	...
	if (
		...
		if (lowerMessage.includes('cash')) {
			...
			methodEmoji = '💵';
			methodDetails =
				'Our server will bring the payment terminal to your table. Exact change is appreciated but not required.';
		} else if (
			lowerMessage.includes('credit') ||
			lowerMessage.includes('debit') ||
			lowerMessage.includes('card')
