  ...
  keywords.forEach(({ words, item: itemName }) => {
    words.forEach((word) => {
      if (lowerMessage.includes(word)) {
        ...
        if (
          menuItem &&
          !foundItems.find((f) => f.id === menuItem.id)
        ) {
...
const detectCategory = (message: string): string | null => {
  ...
  if (
    lowerMessage.match(
      /kids|children|child|for kids|kid menu|kids meal/,
    )
  )
    return "kids";
  if (
    lowerMessage.match(
      /family|group|sharing|share|for sharing|family meal/,
    )
  )
    return "family";
  ...
  if (
    lowerMessage.match(
      /quick|fast|express|ready fast|quick meal/,
    )
  )
    return "quick";
  ...
  if (lowerMessage.match(/bavarian|bayern|bavaria/))
    return "bavarian";
  if (
    lowerMessage.match(/traditional|authentic|classic|regional/)
  )
    return "traditional";
  ...
  if (
    lowerMessage.match(
      /starter|appetizer|begin|first course|to start/,
    )
  )
    return "starter";
  if (
    lowerMessage.match(
      /main|entree|entrée|main course|dinner|lunch/,
    )
  )
    return "main";
  if (
    lowerMessage.match(/dessert|sweet|cake|pastry|after dinner/)
  )
    return "dessert";
  if (
    lowerMessage.match(
      /drink|beverage|beer|wine|water|coffee|espresso/,
    )
  )
    return "drinks";
...
export const generateAIResponse = (
  ...
  if (hasOrderingIntent(lowerMessage)) {
    ...
    if (foundItems.length > 0) {
      ...
      const pairingText =
        pairings.length > 0
          ? `\n\n🍷 **Perfect Pairing Suggestion:**\nMay I recommend our **${pairings[0].name}** ($${pairings[0].price.toFixed(2)})? It complements the ${item.name} beautifully!`
          : "";

      return {
        text: `Excellent choice! ✨\n\nI've added **${item.name}**${quantityText} to your cart.\n\n📋 ${item.description}\n💰 **Price:** $${item.price.toFixed(2)}${pairingText}\n\nWould you like to add anything else, or shall we proceed to checkout?`,
        suggestedItems:
          pairings.length > 0
            ? pairings
            : foundItems.slice(1, 3),
        autoAddedItems,
      };
    } else {
  ...
  if (
    lowerMessage.match(
      /^(hi|hello|hey|good|bonjour|greetings|hola|howdy)/,
    )
  ) {
    const popularItems = menuData.filter(
      (item) => item.popular,
    );
    return {
  ...
  if (
    ...
    !hasOrderingIntent(lowerMessage)
  ) {
    const item = queriedItems[0];
    const pairings = getPairingRecommendations([item]);
    const pairingText =
      pairings.length > 0
        ? `\n\n🍷 **Perfect Pairing:** ${pairings[0].name} ($${pairings[0].price.toFixed(2)})`
        : "";

    return {
      text: `Great question about the **${item.name}**! 🍽️✨\n\n${item.description}\n\n💰 **Price:** $${item.price.toFixed(2)}\n🌿 **Vegetarian:** ${item.vegetarian ? "Yes" : "No"}\n⏱️ **Prep Time:** ${item.prepTime || "10-15 mins"}\n⚠️ **Allergens:** ${item.allergens.length > 0 ? item.allergens.join(", ") : "None"}${pairingText}\n\n${item.popular ? "⭐ **Guest Favorite!** This is one of our most beloved dishes!\n\n" : ""}💡 **To order:** Just say "I'll have the ${item.name}" or "Add it to my cart"!\n\nWould you like to know more or shall I add this to your order?`,
      suggestedItems:
        pairings.length > 0
  ...
  if (
    lowerMessage.match(
      /vegetarian|veggie|no meat|plant.*based|vegan/,
    )
  ) {
    const veggieItems = menuData.filter(
  ...
  if (
    lowerMessage.match(
      /allergen|allergy|gluten|dairy|lactose|nut|egg/,
    )
  ) {
    const allergenType = lowerMessage.includes("gluten")
      ? "gluten"
      : lowerMessage.includes("dairy") ||
          lowerMessage.includes("lactose")
        ? "dairy"
  ...
  if (
    lowerMessage.match(
      /wine|drink|beer|beverage|alcohol|cocktail|coffee|water/,
    )
  ) {
    const drinks = menuData.filter(
  ...
  if (
    lowerMessage.match(
      /main|entree|entrée|dinner|lunch|hearty|filling/,
    )
  ) {
  ...
  if (
    lowerMessage.match(
      /bye|done|that.*all|nothing else|finish|complete/,
    )
  ) {
    if (cart.length > 0) {
      return {
  ...
  if (
    lowerMessage.match(
      /\b(discount|voucher|promo|offer|deal|coupon)s?\b/,
    )
  ) {
    return {
  ...
  if (
    lowerMessage.match(
      /checkout|pay\s*now|payment|proceed.*pay|ready.*pay/,
    )
  ) {
    if (cart.length === 0) {
      return {
  ...
  if (
    lowerMessage.match(
      /calorie|nutrition|healthy|protein|carb|fat|diet/,
    )
  ) {
    return {
