...
import { trainingData, entityPatterns, TrainingExample } from '../data/chatbotTrainingData';
...
export function extractQuantity(input: string): number {
  ...
  const numericMatch = lowerInput.match(/(\d+)\s*(x|order|orders|piece|pieces|portion|portions)?/i);
  if (numericMatch) {
...
export function extractDishName(input: string): { dish: MenuItem | null; confidence: number } {
  const lowerInput = input.toLowerCase();
  let bestMatch: { dish: MenuItem | null; confidence: number } = { dish: null, confidence: 0 };
  ...
  if (bestMatch.confidence < 0.8) {
    for (const [dishName, variations] of Object.entries(entityPatterns.dishes)) {
...
export function extractCategory(input: string): string | null {
  ...
  for (const [category, keywords] of Object.entries(entityPatterns.categories)) {
...
export function extractModifications(input: string): string[] {
  ...
  if (withoutMatch) {
    modifications.push(...withoutMatch.map(m => m.replace('without', 'no').trim()));
  }
...
export function recognizeIntent(userInput: string): RecognizedIntent {
  ...
  if (modifications.length > 0) result.entities.modification = modifications.join(', ');
  if (occasion) result.entities.occasion = occasion;
  
  // Pattern-based intent recognition
  
  // ORDERING INTENTS
  if (/i'?ll have|can i get|i want|give me|add|order/i.test(lowerInput) && dishMatch.dish) {
  ...
  // MENU BROWSING
  if (/show|see|view|browse|display/i.test(lowerInput) && /menu/i.test(lowerInput)) {
  ...
  // DISH INFORMATION
  if (dishMatch.dish && /what is|tell me about|how much|what.*come|ingredients/i.test(lowerInput)) {
  ...
  if (/checkout|pay|payment|bill|check/i.test(lowerInput)) {
...
function findBestTrainingMatch(input: string): { intent: string; confidence: number } | null {
  const inputWords = input.toLowerCase().split(' ').filter(w => w.length > 2);
  let bestMatch: { intent: string; confidence: number } | null = null;
  
  trainingData.forEach(example => {
    const exampleWords = example.userInput.toLowerCase().split(' ').filter(w => w.length > 2);
    
    // Calculate word overlap
    const matchingWords = inputWords.filter(word => exampleWords.includes(word));
    const confidence = matchingWords.length / Math.max(inputWords.length, exampleWords.length);
...
export function refineIntentWithContext(
  ...
  if (conversationContext.lastIntent === 'VIEW_MENU' && recognizedIntent.matchedDish) {
